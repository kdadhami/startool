...
$mysql_data31 = mysql_query($query31, $mysql_link) or die ("#2 Could not query: ".mysql_error());
...
if ($submit == "Submit") {
   if (isset($update1)) {
    ...
    if (isset($area_cnt)) {
        for ($h=1;$h<=$area_cnt;++$h) {
           ...
           //print($queryy);                 
		   $mysql_datay = mysql_query($queryy, $mysql_link) or die ("#5 Could not query: ".mysql_error());
        }    
    }
   }
   ...
   if (isset($update2)) {
    while (list($key2) = each($update2)) {
        ...
        $xstart_date_2[$key2]  = mktime(0,0,0,$xms4[$key2],$xds4[$key2],$xys4[$key2]);
        $xend_date_2[$key2]    = mktime(0,0,0,$xme4[$key2],$xde4[$key2],$xye4[$key2]);        
        ...
        $queryy = "UPDATE ".$name.".pdp_execution
                      SET start_date       = '$xstart_date[$key2]', 
                          ...
                          start_date_2     = '$xstart_date_2[$key2]', 
                          end_date_2       = '$xend_date_2[$key2]', 
                          updated_by       = '$usr',
                          comments         = '$ycomments[$key2]',
                          running_comments = '$yrunning_com[$key2]'
		            WHERE execution_id     = '$key2' ";
		   //print($queryy);         
		   $mysql_datay = mysql_query($queryy, $mysql_link) or die ("#6 Could not query: ".mysql_error());
           while (list($key3) = each($yiteration_cnt[$key2])) {
                  ...
                  //print($queryz);            
                  $mysql_dataz = mysql_query($queryz, $mysql_link) or die ("#7 Could not query: ".mysql_error());                
           }
    }
   }   
}
   ...
   if ($rowcntx == 1) {
             ...
             if ($rowcnty == 0) {
                ...
                for ($d=1;$d<=$dept_cnt ; ++$d) {
                     ...
                     if ($dept_code == "UAT"){
                         ...
                         $mysql_datai = mysql_query($queryi, $mysql_link) or die ("#10 Could not query: ".mysql_error());
                     }
                }
             } 
             // ---------------------------------------------------
       ...
       while($row42 = mysql_fetch_row($mysql_data42)) {
               ...
               print("
                      ...
                      <tr>
                       <td bgcolor=\"#99CCFF\" align=\"left\"><font color=\"#330099\">First Pass Testing<br>Start Date (dd-mm-yyyy)<br>End Date (dd-mm-yyyy)</font></td>
                       <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:300px;\">
                        <font color=\"#330099\">
               ");
               if ($ysdt == "00-00-0000") {
                   $colr = "#FFFF00";
               ...
               //if ($yedt == "00-00-0000") {
                   print("<br>
                          <select align=\"left\" name=\"xde[$yid]\" style=\"color: #000000; font-weight: normal; background-color: $colr;\"> ");
               //} else {
               ...
               print("   </font>
                      ...
                      <tr>
                       <td bgcolor=\"#99CCFF\" align=\"left\"><font color=\"#330099\">Second Pass<br>Start Date (dd-mm-yyyy)<br>End Date (dd-mm-yyyy)</font></td>
                       <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:300px;\">
                        <font color=\"#330099\">
               ");
               if ($ysdt4 == "00-00-0000") {
