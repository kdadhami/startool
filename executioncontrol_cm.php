    ...
    while (list($key2) = each($update2)) {
        ...
        if ($rowcnt9 > 0){
            ...
            while($rowx9 = mysql_fetch_row($mysql_data9)) {
              ...
              //for ($e=1;$e<=$rowcnt9 ; ++$e) { 
                  ...
                  //if ($xact_end_date[$key2][$e] <  $xact_start_date[$key2][$e]) {
                  //    $xact_end_date[$key2][$e] =  $xact_start_date[$key2][$e];
                  //    $yactual_testing_days[$key2][$e] = 0;  
                  //    $yactual_weekend_days[$key2][$e] = 0;  
                  //} 
                  
                  
                  //$xact_start_date[$key2][$e] = $xstart_date[$key2][$e];
                  //print($xstart_date[$key2][$e]."<br>"); 
                  //print($xend_date[$key2][$e]."<br>");
                  //print("xact".$xact_end_date[$key2][$e]."<br>");             
                  ...
                  //    $xend_date[$key2][$e] = 0;
                  //}            
                  //if ($xact_end_date[$key2][$e] == 1167627600) {
                  //    $xact_end_date[$key2][$e] = 0;
                  //}            
                  ...
                  //if ($rowcnt96 <> 0){
                      ...
                      //while($rowx96 = mysql_fetch_row($mysql_data96)) {
                            ...
                            //if ($eit == $key3){ 
                                ...
                                //print($nback_to_build."<br>");
                                //print($nstart_date."<br>");
                                //print($nend_date ."<br>");
                                //print($nact_end_date ."<br>");
                                //print($nactual_testing_days."<br>");
                                //print($nback_to_build);
		...
		if (isset($update3)) {
			...
			while (list($key3x) = each($update3)) {
                   ...
                   //print("Previous Baseline Totoal: ".$prv_baseline_time[$key2x][$key3x]."<br>Previous Incremental Total: ".$prv_incremental_time[$key2x][$key3x]."<br>");				   
				   
				   //print("Milestone: ".$ymilestone_time[$key2x][$key3x]."<br>");
				   //print("Adjustment: ".$yajustment[$key2x][$key3x]."<br>");
				   if ($yajustment[$key2x][$key3x] == "NULL"){
				       print("NULL IS NO<br>");
				   } else {
					 //print("Do adjustments<br>");
					 //tablesrc = O = Original i.e. milestone_surrogates entries
					 //tablesrc = A = Archive i.e. milestone_surrogates_archive entries
					 $ajusttype              = substr($yajustment[$key2x][$key3x],0,1);
					 if ($ajusttype == "B"){
					     $ajustdesc = "Baseline";
					 }
					 if ($ajusttype == "R"){
                     ...
                     $tblsrc                 = substr($yajustment[$key2x][$key3x],1,1);
					 $occrnc                 = substr($yajustment[$key2x][$key3x],2,strlen($yajustment[$key2x][$key3x])-1);
					 $updoccrnc              = $occrnc;
					 $updadj[$key2x][$key3x] = $occrnc;
					 $updtbl[$key2x][$key3x] = $tblsrc;
                     //print("Type:".$ajustdesc."<br>Tablesrc: ".$tblsrc."<br>Occrns: ".$occrnc."<br>".$ajusttype."<br>");
					 //////////////////////////////////////////////////////////////////////
					 ///////////////////////// SECTION ADJUTMENT //////////////////////////
					 //////////////////////////////////////////////////////////////////////
					 if ($tblsrc == "O"){
                         ...
                         while($row44c = mysql_fetch_row($mysql_data44c)) {
							   ...
							   $ybaseline_time[$key2x][$key3x]               = stripslashes($row44d[5]);
							   //print("Milestone 2: ".$ymilestone_time[$key2x][$key3x]."<br>");
							   //$yincremental_time_1[$key2x][$key3x]        = $yincremental_time_1[$key2x][$key3x];
							   if ($ajusttype == "B"){
							       $ybaseline_time_adj[$key2x][$key3x]       = 0;
								   $yincremental_time_adj[$key2x][$key3x]    = $yincremental_time_1[$key2x][$key3x];
								   //print("2<br>");
							   } 
							   if ($ajusttype == "R"){
							      if ($occrnc >=1){
                                      ...
                                      while($row44e = mysql_fetch_row($mysql_data44e)) {
											...
											if ($prv_baseline_time[$key2x][$key3x] == 0){
											    $ybaseline_time[$key2x][$key3x]      = 0;
											} else {
										        $ybaseline_time[$key2x][$key3x]      = stripslashes($row44e[6]);
											}
								  ...
								  $ybaseline_time_adj[$key2x][$key3x]          = $ybaseline_time[$key2x][$key3x];
				   ...
				   //print($yeffort_type[$key2x][$key3x]."<br>");
                   if ($yajustment[$key2x][$key3x] == "NULL"){
				      print("101<br>");
			          ...
			          if (($ymilestone_type[$key2x][$key3x] == "F") or ($ymilstone_type[$key2x][$key3x] == "N")){
                          if (empty($yiteration_cnt[$key2x][$key3x])){                                                                                                                // If user has not selected any iteration                      
							  ...
							  if ($ushort_desc == "CMGT"){
							      if ($yeffort_type[$key2x][$key3x] == "B"){
							          $ybaseline_time[$key2x][$key3x]    = 0;
                                  } 
 							      if ($yeffort_type[$key2x][$key3x] == "R"){
				      	              $yincremental_time[$key2x][$key3x] = 0;          // Incremental Time for F/N Retrofit Parities 
                                  } 
							  } else {
							    $ybaseline_time[$key2x][$key3x]          = addslashes($prv_baseline_time[$key2x][$key3x]);
							    $yincremental_time[$key2x][$key3x]       = 0;          // Incremental Time for F/N Retrofit Parities 
							  }
				              $ytotal_time[$key2x][$key3x]          = $ybaseline_time[$key2x][$key3x] + $yincremental_time[$key2x][$key3x];                                           // Total Time for F/N Retrofit Parities
                          } else {
                              //print("2<br>".$yiteration_cnt[$key2x][$key3x]."<br>".$ymilestone_time[$key2x][$key3x]."<br>");
						      $ziteration_cnt                       = addslashes($yiteration_cnt[$key2x][$key3x]);                                                                    // Iteration for F/N Retrofit Parities 
							  if ($ushort_desc == "CMGT"){
							      if ($yeffort_type[$key2x][$key3x] == "B"){
								      //print("A<br>");
							          $ybaseline_time[$key2x][$key3x]    = $yiteration_cnt[$key2x][$key3x] * $ymilestone_time[$key2x][$key3x];
                                      $yincremental_time[$key2x][$key3x] = $prv_incremental_time[$key2x][$key3x];
									  //print("Baseline:".$ybaseline_time[$key2x][$key3x]."<br>"."Incremental:".$yincremental_time[$key2x][$key3x]."<br>");
                                  } 
 							      if ($yeffort_type[$key2x][$key3x] == "R"){
								      //print("B<br>");
								      $ybaseline_time[$key2x][$key3x]    = addslashes($prv_baseline_time[$key2x][$key3x]);
                              ...
                              //print("Total:".$ytotal_time[$key2x][$key3x]."<br>");
					     }
				      }
			          if ($ymilestone_type[$key2x][$key3x] == "V"){
                          ...
                          //print($ziteration_cnt."<br>");
                          //exit;						  
						  if ($ushort_desc == "CMGT"){
							      if ($yeffort_type[$key2x][$key3x] == "B"){
							          $ybaseline_time[$key2x][$key3x]    = ($ymilestone_hrs[$key2x][$key3x]*60) + $ymilestone_min[$key2x][$key3x];
                                      $yincremental_time[$key2x][$key3x] = $prv_incremental_time[$key2x][$key3x];
                                  } 
 							      if ($yeffort_type[$key2x][$key3x] == "R"){
								      $ybaseline_time[$key2x][$key3x]    = addslashes($prv_baseline_time[$key2x][$key3x]);
				      	              $yincremental_time[$key2x][$key3x] = ($ymilestone_hrs[$key2x][$key3x]*60) + $ymilestone_min[$key2x][$key3x];                                                        // Incremental Time for F/N Retrofit Parities 
                                  } 
						  } else {
							    $ybaseline_time[$key2x][$key3x]          = addslashes($prv_baseline_time[$key2x][$key3x]);
							    $yincremental_time[$key2x][$key3x]       = ($ymilestone_hrs[$key2x][$key3x]*60) + $ymilestone_min[$key2x][$key3x];                                                        // Incremental Time for F/N Retrofit Parities 
					  ...
					  $yadjust_occurance                            = 0;
                          ...
                          //print("Baseline adjustment: ".$ybaseline_time_adj[$key2x][$key3x]."<br>");
                          $ybaseline_time[$key2x][$key3x]           = $ybaseline_time_adj[$key2x][$key3x];
                          //print("Baseline : ".$ybaseline_time[$key2x][$key3x]."<br>");
                          $yincremental_time[$key2x][$key3x]        = $yincremental_time_adj[$key2x][$key3x];	                                                                      // Adjusted Incremental Time
                          //print("Incremental Adjustment: ".$yincremental_time_adj[$key2x][$key3x]."<br>");
					      ...
					      if ($ajusttype == "B"){
                              //print("Previous ".$prv_total_time[$key2x][$key3x]."<br>Baseline: ".$ybaseline_time[$key2x][$key3x]."<br>");
 						      $ytotal_time[$key2x][$key3x]          = $prv_incremental_time[$key2x][$key3x];                                              // Adjusted Total Time
                              //print("ytotal_time: ".$ytotal_time[$key2x][$key3x]."<br>");
						  }
					      if ($ajusttype == "R"){
                              //print("Previous ".$prv_total_time[$key2x][$key3x]."<br>Baseline: ".$ybaseline_time[$key2x][$key3x]."<br>");
							  //print(-1*($yincremental_time_adj[$key2x][$key3x])."<br>");
							  if ($prv_total_time[$key2x][$key3x] == (-1*($yincremental_time_adj[$key2x][$key3x]))){
							      $ytotal_time[$key2x][$key3x]          = $prv_total_time[$key2x][$key3x];
							  } else {
						          $ytotal_time[$key2x][$key3x]          = $prv_total_time[$key2x][$key3x] - $yincremental_time_adj[$key2x][$key3x];   // Adjusted Total Time
							  }
                              //print("ytotal_time: ".$ytotal_time[$key2x][$key3x]."<br>");
					      }
				   ...
				   $last_adjust_occurance             = $prv_adjust_occurance[$key2x][$key3x];       
				   // Calculate Net Baseline
				   $netbaseline                       = $ybaseline_time[$key2x][$key3x];                                           //$prv_baseline_time[$key2x][$key3x];  
                   // Baseline
				   if ($prv_baseline_time[$key2x][$key3x] == 0){
				     ...
				     if($ushort_desc == "CMGT"){
					    if ($netbaseline == 0){
					        ...
					        (float)$ybase_variance        = round((($netbaseline/$netbaseline)*100),2);
						}
					 } else {
				  ...
				  if ($yajustment[$key2x][$key3x] <> "NULL"){
				      if ($ajusttype == "B"){
					      $netincremental                    = $prv_incremental_time[$key2x][$key3x];
						   //print("701-A<br>");
					  } else {
                          //if ($prv_baseline_time[$key2x][$key3x] == 0){
				              if ($prv_incremental_time[$key2x][$key3x] == (-1*($yincremental_time_adj[$key2x][$key3x]))){
				                  $netincremental                    = 0;
								   //print("701-B<br>");
					          } else {
                                  $netincremental                    = $prv_incremental_time[$key2x][$key3x] - (-1*($yincremental_time_adj[$key2x][$key3x]));
								   //print("701-C<br>");
                              }					   
					          //print("701<br>");
				          //}
				       ...
				       if ($yincremental_time[$key2x][$key3x] == $prv_incremental_time[$key2x][$key3x]){
				           $netincremental = $prv_incremental_time[$key2x][$key3x];
						   //print("701-D<br>");
					   } else {
					       //if ($prv_incremental_time[$key2x][$key3x] == (-1*($yincremental_time_adj[$key2x][$key3x]))){
				   ...
				   //print("------<br>"."Baseline:".$netbaseline."<br>"."Incremental:".$netincremental."<br>");
                   //print("Total:".$nettotal."<br>");
				   // Incremental
				   if ($yincremental_time[$key2x][$key3x] == 0){
				       (float)$yincremental_variance      = 0;
				   } else {
				     if($ushort_desc == "CMGT"){
					    if ($netbaseline == 0 or $netincremental == 0){
						    (float)$yincremental_variance  = 0;
						} else {
					      (float)$yincremental_variance  = round((($netincremental/$netbaseline)*100),2);
						}
					 } else {
				   ...
				   if ($nettotal == $last_total_time){ 
				       //print("Net Total: ".$nettotal."<br> Last Total Time: ".$last_total_time."<br>");
					   $updchk  = 0;
				   } else {
				       //print("Net Total: ".$nettotal."<br> Last Total Time: ".$last_total_time."<br>");
					   $updchk  = 1;
...
// -------------------------------------
   ...
   if ($rowcntx == 1) {
       ...
       while($row42 = mysql_fetch_row($mysql_data42)) {
               ...
               if (($rowcnt9 == 0) or ($rowcnt9 == 1)){
			             if ($rowcnt9 == 1){
			                 while($row9x = mysql_fetch_row($mysql_data9)) {
						           $qtest_it = stripslashes($row9x[0]);        
						     }
							 if ($qtest_it == 1){
							     $rtest_it = 2;
							 }
                         ...
                         // Inserting a missing iteration record						 
						 if ($rowcnt9 == 1){
                            $ytest_iteration = 2;
                            for ($yit=1;$yit<=$ytest_iteration; ++$yit) {
                              if ($yit == $rtest_it){							
               ...
               if ($rowcnt95 <> 0){
                   ...
                   while($rowx95 = mysql_fetch_row($mysql_data95)) {
                         ...
                         //}
                            ...
                            while($rowx29 = mysql_fetch_row($mysql_data29)) {
                                  ...
                                  //print($yback_to_build_sum."<br>");
                                    ...
                                    if ($zit == 1){
                                        $iter = "Testing Cycle Pre-Prod";
                                    }
                                    if ($zit == 2){
                                        $iter = "Testing Cycle Production";
                                    }
                                    print("
                                           <tr>
                                            <td colspan=\"4\" align=\"center\" valign=\"middle\" bgcolor=\"#99CCFF\" style=\"width:30%;\">
                                             <strong>
                                              <font color=\"#330099\">
                                               <a> 
                                                $iter
                                    ...
                                    //if ($d5 > 0) {
                                        ...
                                        //print("    
                                        //    <td align=\"left\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:30%;\">
                                        //     <select align=\"left\" name=\"yactual_testing_days[$yid][$zit]\">
                        ...
                        //          TBD
                                    ...
                                    // ----------------------------------------------------------------------------------------------------------- 
                                    print("
                                            <td bgcolor=\"#99CCFF\" align=\"left\" style=\"width:15%;\"><font color=\"#330099\">Effort<br>(% of Days Utilized)</font></td>
                                            <td align=\"left\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:30%;\">
                                             <font color=\"#330099\">
                                               $usd
                                             </font> 
                                    ...
                                    ");
                                    if ($ushort_desc == 'MO'){
                                      print(" <a>$yback_to_build</a>         
                                              <input type=\"hidden\" name=\"yback_to_build[$yid][$zit]\" value=\"$yback_to_build\"><a>&nbsp($uissue_area) ONLY</a>
                                      "); 
                                    } else {
                                      print("        
               ...
               print("
                      ...
                      <tr>
                       ...
                       <td align=\"left\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:30%;\">
                         ...
                         <input type=\"hidden\" name=\"yinvoice_cnt[$yid]\" value=\"$yinvoice_cnt\">
                        </font> 
                       </td>
                       ...
                       <td align=\"left\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:30%;\">
                         ...
                         <input type=\"hidden\" name=\"ybillrun_cnt[$yid]\" value=\"$ybillrun_cnt\">
                        </font> 
                       </td>
                      </tr>
                      <tr>
                       ...
                       <td align=\"left\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:30%;\">
                         ...
                         <input type=\"hidden\" name=\"yppw_update[$yid]\" value=\"$yppw_update\">
                       </td>
			   ...
			   if ($uissue_area == 'ENTERPRISE UAT'){
                ...
                <tr>
                 <td colspan=\"4\" border=\"1\" bgcolor=\"#CCFFCC\" align=\"left\" valign=\"top\" style=\"word-wrap: break-word; word-break:break-all; width:100%; height:100px;  scroll-x: auto;\">
                  ...
                  <div style=\"word-wrap: break-word; word-break:break-all; width:100%; height:100px; overflow: auto; background-color: #CCFFCC;\">
                   <p>
               ");
       ...
       if ($rowcnt44 == 0) {
		   ...
		   if ($ushort_desc == "CMGT"){
               print("		   
                 <td bgcolor=\"#CCCC99\" align=\"center\" colspan=\"3\" ><font color=\"#330099\">Enter Rework<br>Effort</font></td>
               "); 		   
		   } else {
               print("		   
           ...
           print("		   
                ...
                <tr>
				 ...
				 <td bgcolor=\"#FFCC66\" align=\"center\"><font color=\"#330099\">Iterations<br>(Lines of Code)</font></td>
		   ");
		   if ($ushort_desc == "CMGT"){
               print("		   
                 <td bgcolor=\"#FFCC66\" align=\"center\"><font color=\"#330099\">Select<br>B=Baseline<br>R=Rework</font></td>
               "); 		   
		   } else {
		   }
           print("		   
           ...
           if ($rowcnt34 == 0) {  
               for ($d=1;$d<=$mil_cnt;++$d) {
                    if ($mil_ind[$d] == 1) {
					    ...
					    // Make Baseline start from 0 when use is from Configuration Management
					    if ($ushort_desc == "CMGT"){
						    $mil_time[$d] = 0;
						} 
           ...
           while($row44 = mysql_fetch_row($mysql_data44)) {
				...
				$row_base[$eseq_a]                = $ybaseline_time;
				$row_incr[$eseq_a]                = $yincremental_time;
				$net_total                        = $net_total + ($ybaseline_time + $yincremental_time);				
                ...
                while($row44_a = mysql_fetch_row($mysql_data44_a)) {
					  ...
					  $row_base[$eseq_a]                                  = $ybaseline_time_a2[$eseq_a];
					  $row_incr[$eseq_a]                                  = $yincremental_time_a2[$eseq_a];
					  $net_total                                          = $net_total + ($ybaseline_time_a2[$eseq_a] + $yincremental_time_a2[$eseq_a]);
					...
					if ($ushort_desc == "CMGT"){
                        print("					
					           <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"font-size: 10px;\">
                                <font color=\"#330099\">
								 <input type=\"text\" name=\"yiteration_cnt[$yid][$zid]\" value=\"0\" size=\"4\" maxlength=\"4\">
                                </font> 
                               </td>
					           <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"font-size: 10px;\">
                                <font color=\"#330099\">
								 <select align=\"center\" name=\"yeffort_type[$yid][$zid]\">
						");
			            for ($wrk=0;$wrk<=2; ++$wrk) {
					       if($ybaseline_time == 0){
                             if($wrk == 1){						   
                                print("<option value=\"$wrk_effort[$wrk]\">$wrk_effort[$wrk]</option> ");
							 }
						   } else {
                              if($wrk == 2){						   
                                print("<option value=\"$wrk_effort[$wrk]\">$wrk_effort[$wrk]</option> ");
			    ...
			    if ($ymilestone_type == "V"){
				  ...
				  ");
				  if ($ushort_desc == "CMGT"){
				      print("  <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"font-size: 10px;\">
                                <font color=\"#330099\">
								 <select align=\"center\" name=\"yeffort_type[$yid][$zid]\">
					  ");
			          for ($wrk=0;$wrk<=2; ++$wrk) {
                  ...
                  ");				  
			        ...
			        if ($ymilestone_type == "N"){
			            print("
                           ...
                           <td align=\"center\" valign=\"middle\" bgcolor=\"#FFFFCC\">
                            <font color=\"#330099\">
							 <input type=\"hidden\" name=\"yiteration_cnt[$yid][$zid]\" value=\"0\">
                            </font> 
                           </td>
                        ");
				        if ($ushort_desc == "CMGT"){
			             ...
			             for ($his=0;$his<=$eseq_a; ++$his) {
							  ...
							  //if ($row_milflg[$his] == "V"){
							      if ($his <> 0){
								     //print($his."<br>");
									  ...
									  // Baseline
							          $row_base_hrs[$his]         = intval($row_base[$his]/60);
                                      $row_base_mins[$his]        = $row_base[$his]%60;
									  // Incremental
                                      ...
                                      // --------
                                      //Baseline
					                  if (strlen($row_base_hrs[$his]) == 1){
					                      $row_base_hrs_x[$his] = "0".$row_base_hrs[$his];
					                  } else {
									      $row_base_hrs_x[$his] = $row_base_hrs[$his];
									  }
					                  if (strlen($row_base_mins[$his]) == 1){
					                      $row_base_mins_x[$his] = "0".$row_base_mins[$his];
					                  } else {
									      $row_base_mins_x[$his] = $row_base_mins[$his];
									  }
                                      // Incremental
								      ...
								      if ($his == $eseq_a){
								        ...
								        if(($row_incr[$his] == $row_incr[$his2]) and ($row_base[$his] <> 0)){
										    $row_entry[$his]  = "B";
											//$bcnt             = $bcnt + 1;
											$row_hrs_x[$his]  = $row_base_hrs_x[$his];
											$row_mins_x[$his] = $row_base_mins_x[$his];
											//print("1<br>");
										}
								        if($row_incr[$his] <> $row_incr[$his2]){
										   $row_entry[$his]  = "R";
										   //$rcnt             = $rcnt + 1;
										   $row_hrs_x[$his]  = $row_incr_hrs_x[$his];
										   $row_mins_x[$his] = $row_incr_mins_x[$his];
										   //print("2<br>");
										}
										$ttcnt = $bcnt + $rcnt;
									  }
									  $mil_row_flag[$his]      = $row_entry[$his].$mil_row_flag[$his];
									  $mil_history_rows[$his]  = $row_itcnt[$his]." | ".$row_hrs_x[$his].":".$row_mins_x[$his]." | ".$row_entry[$his];
								      //print($mil_history_rows[$his]."<br>");
							      }
							  //}
						      if ($his <> $eseq_a){    // do not display the 0 occurance entry; $eseq_a begins with 0 which is the initial occurance when a record is created in milestone_surrogate table.
							      if ($row_adjust[$his] <> 0){
								    ...
								    if ((($row_parity[$his] == "N" or $row_parity[$his] == "X") and ($row_itcnt[$his] <> 0)) or ($his == 0)){ 
									     if ($row_parity[$his] == "X"){
										 } else {
	   ...
	   // The hidden fields below are for PDP Tracker section and not Rework Effort Section
       print(" <tr>
                <td>
