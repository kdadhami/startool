...
//---------
   ...
   if ($rowcntx == 1) {
          ...
          print("
           <form method=\"post\" action=\"./setup_pdp2_x.php?pdp=$pdp&&start=$start\">
            <table border='0' align=\"center\" scroll=\"yes\">
             ...
             <tr>
              <td bgcolor=\"#99CCFF\" align=\"center\"><font color=\"#330099\">PDP Description</font></td>
     ...
     for ($f=1;$f<=$pcat_cnt; ++$f) {
         if ($pcat_id[$f] == -1)
         {
             print(" <option value=\"$pcat_id[$f]\" disabled=\"disabled\" style=\"color: blue; font-weight: normal; background-color: #CCCCCC;\">$pcat[$f]</option> ");
         } 
         if ($pcat_id[$f] == 0)
         {
         ...
         if (($pcat_id[$f] <> -1) && ($pcat_id[$f] <> 0))
         {
             if ($xpdp_category_id == $pcat_id[$f])  {
                  if ($pcat_id[$f] <> 0) {
                      print(" <option selected value=\"$pcat_id[$f]\">$pcat[$f]</option> "); 
