...
$xreport_pth = "./reports/";
$xreport_nam = "SUMMARY_REPORT-".$ydate3.".htm";
//print ($reportname); 

...
if ($start == 1) {
   ...
   //                           11 a.build_pw           as 'BUILD PAPERWORK',
   //                           12 a.pdp_category       as 'CATEGORY',
   //                           13 a.complexity_factor  as 'COMPLEXITY',
   //                           14 a.revenue_factor     as 'REVENUE IMPACT',
   //                           15 a.customer_factor    as 'CUSTOMER IMPACT',
   //                           16 a.past_factor        as 'PAST PERFORMANCE',
   //                           17 a.bills_run          as 'BILL RUNS',
   //                           18 a.invoices_generated as 'INVOICE GENERATED',
   //                           19 a.ppw_changes        as 'PPW CHANGES',
   //                           20 a.launch_in_jeopardy as 'LAUNCH IN JEOPARDY',
   //                           21 a.issues_created     as 'ISSUES CREATED',
   //                           22 b.tested             as 'UAT TESTED',
   //                           23 c.tested             as 'RA TESTED',
   //                           24 1                    as 'FILTER' 
   //$cnd1 = "HIGH";
   //$cnd2 = "MEDIUM";
   //$cnd3 = "LOW";
   ...
   $querypdp      = "  select 'tgt_pdp_main'                    as 'DUMMY',
                        ...
                        where trim(a.pdp_status) <> 'Cancelled'
                          ...
                          " . ($bld_pw_ind != 'ALL' ? " and (a.build_pw = '" . $bld_pw_ind . "')" : "");
   //                
   //a.etl_id = '$yetl_id'
   //                       and (a.pdp_launch >= '2010-09-01' or (a.pdp_launch = '0000-00-00' and a.issues_created <> 0))
   //                       and ((a.pdp_launch >= '$xldate' and a.pdp_launch <= '$xldate2') or (a.pdp_launch = '0000-00-00' and a.pdp_launch > '2010-09-01')) 
   ...
   while($rowp  = mysql_fetch_row($mysql_datap)) {
         ...
         if ($total_pdp == 1){
             for ($pc=1;$pc<=$pcol;++$pc) {
                  ...
                  if ($pc == 0 || ($pc >= 3 && $pc <= 4) || ($pc >= 6 && $pc <= 9) || ($pc >= 13 && $pc <= 19) || $pc == 24){
                      $pdp_head_use[$pc] = 0;
                      $pc3a = $pc3a + 1;
                  } else {
                      $pdp_head_use[$pc] = 1;
   ...
   if ($fltr3 == 1){
     ...
     for ($tp=1;$tp<=$total_pdp;++$tp) {
        for ($pc=1;$pc<=$pcol;++$pc) {
                 if ($pc == 10 && $pdp[$tp][$pcol] == 1){                       // Change the $pc value for different filters, $pc = 10+12 means concat(SCOPING." - ".PDP_CATEGORY)
                     $valx = $pdp[$tp][$pc]." - ".$pdp[$tp][$pc+2];
                     //if ($pdp[$tp][$pc] <> $pdpcat){
                     if ($valx <> $pdpcat){
                         $fltcnt3 = $fltcnt3 + 1;
                         $pdp[$tp][$pcol] = 0;
   ...
   if ($fltr2 == 1){
     ...
     for ($tp=1;$tp<=$total_pdp;++$tp) {
        for ($pc=1;$pc<=$pcol;++$pc) {
                 if ($pc == 23 && $pdp[$tp][$pcol] == 1){                       // Change the $pc value for different filters, $pc = 23 means RA_IND
                     $valx = $pdp[$tp][$pc];
                     //if ($pdp[$tp][$pc] <> $pdpcat){
                     if ($valx <> $ra_ind){
                         $fltcnt2 = $fltcnt2 + 1;
   ...
   $sum1[17]    = 0; //BILLS RUN 
   $analsis[18] = "&nbsp";
   $sum1[18]    = 0; //INVOICE GENERATED 
   $analsis[19] = "&nbsp";
   $sum1[19]    = 0; //PPW CHANGES 
   $analsis[20] = "&nbsp";
   $sum1[20][1] = 0; // LAUNCH IN JEOPARDY WHEN YES
   $sum1[20][2] = 0; // LAUNCH IN JEOPARDY WHEN NO
   $analsis[21] = "&nbsp";
   $sum1[21]    = 0; //ISSUES CREATED
   $sum1x[21][1]= 0;
   $sum1x[21][2]= 0;
   $sum1x[21][3]= 0;
   $sum1x[21][4]= 0;
   ...
   for ($tp=1;$tp<=$rowcount;++$tp) {
         for ($pc=1;$pc<=$pcol;++$pc) {
                ...
                if ($pc == 10){
				    ...
				    if ($xpdp[$tp][$pc] == "IN SCOPE" && $xpdp[$tp][22] == "YES") {
					    $sum1[$pc][1] = $sum1[$pc][1] + 1;
					}
				    if ($xpdp[$tp][$pc] == "NOT SET" && $xpdp[$tp][22] == "YES" ) {
					    $sum1[$pc][1] = $sum1[$pc][1] + 1; 
					}
				    if ($xpdp[$tp][$pc] == "OUT OF SCOPE" && $xpdp[$tp][22] == "YES") {
					    $sum1[$pc][1] = $sum1[$pc][1] + 1;
					}
				    if ($xpdp[$tp][$pc] == "OUT OF SCOPE" && $xpdp[$tp][22] == "NO") {
					    $sum1[$pc][2] = $sum1[$pc][2] + 1;
					}
				    if ($xpdp[$tp][$pc] == "NOT SET" && $xpdp[$tp][22] == "NO") {
					    $sum1[$pc][3] = $sum1[$pc][3] + 1;
					}
					
  				    //switch ($xpdp[$tp][$pc] && $xpdp[$tp][22]) {
                    //        case ("IN SCOPE" && $xpdp[$tp][22] == "YES"):      // COUNTED AS IN-SCOPE
                    //        $sum1[$pc][1] = $sum1[$pc][1] + 1;
                    //        break;
                    //        case ("NOT SET" && $xpdp[$tp][22] == "YES"):       // ALSO COUNTED AS IN-SCOPE
                    //        $sum1[$pc][1] = $sum1[$pc][1] + 1;
                    //        break;
                    //        case ("OUT OF SCOPE" && $xpdp[$tp][22] == "YES"):  // ALSO COUNTED AS IN-SCOPE
                    //        $sum1[$pc][1] = $sum1[$pc][1] + 1;
                    //        break;
                    //        case ("OUT OF SCOPE" && $xpdp[$tp][22] == "NO"):   // COUNTED AS OUT-OF-SCOPE
                    //        $sum1[$pc][2] = $sum1[$pc][2] + 1;
                    //        break;
                    //        case ("NOT SET" && $xpdp[$tp][22] == "NO"):        // COUNTED AS NOT-SET 
                    //        $sum1[$pc][3] = $sum1[$pc][3] + 1;
                    //        break;
					//}
                    ...
                    //Analysis
                    $analsisi[$pc] = "<u>IN SCOPE</u><br><br>".$sum1[$pc][1]." PDPs<br>(".$sumy[$pc][1]."%)";
                    $analsiso[$pc] = "<u>OUT OF SCOPE</u><br><br>".$sum1[$pc][2]." PDPs<br>(".$sumy[$pc][2]."%)";
                    $analsisn[$pc] = "<u>NOT SET</u><br><br>".$sum1[$pc][3]." PDPs<br>(".$sumy[$pc][3]."%)";
                }
                if ($pc == 11){
                      $analsis[$pc]= "Build Paperwork analysis here!!";
                }
                // 13 COMPLEXITY
                // 14 REVENUE IMPACT
                // 15 CUSTOMER IMPACT
                ...
                if ($pc >= 13 && $pc <= 16){
                    ...
                    $analsis[16] = "<u>HIGH</u><br>".$sum1[16][1]."<br>(".$sumy[16][1]."%)"."<br><br><u>MEDIUM</u><br>".$sum1[16][2]."<br>(".$sumy[16][2]."%)"."<br><br><u>LOW</u><br>".$sum1[16][3]."<br>(".$sumy[16][3]."%)"."<br><br><u>NOT SET</u><br>".$sum1[16][4]."<br>(".$sumy[16][4]."%)";
                }
                // 17 BILL RUNS
                // 18 INVOICE GENERATED
                // 19 PPW CHANGES
                // 21 ISSUES CREATED
                if (($pc >= 17 && $pc <=19) || $pc == 21){
                    if ($xpdp[$tp][$pc] > 0){
                ...
                if ($pc == 11){
                    ...
                    if ($xpdp[$tp][$pc] == "NOT SET"){
                        $sum1[$pc][0] = $sum1[$pc][0] + 1;
                    }
                    $sum_bld_pw = $sum1[$pc][1] + $sum1[$pc][2] + $sum1[$pc][0];
    				if ($sum_bld_pw == 0){
					  $prc1[$pc] = 0;
					  $prc2[$pc] = 0;
                                          $prc0[$pc] = 0;
					    ...
					    if ($sum1[$pc][1] == 0){
						    ...
						    $prc1[$pc] = round(($sum1[$pc][1]/$sum_bld_pw)*100,2); 
						}
					    if ($sum1[$pc][2] == 0){
						    $prc2[$pc] = 0;
						} else {
						    $prc2[$pc] = round(($sum1[$pc][2]/$sum_bld_pw)*100,2); 
						}
					    if ($sum1[$pc][0] == 0){
						    $prc0[$pc] = 0;
						} else {
						    $prc0[$pc] = round(($sum1[$pc][0]/$sum_bld_pw)*100,2); 
						}
					}

                    $analsis[$pc] = "YES = ".$sum1[$pc][1]."<br>(".$prc1[$pc].")%<br><br>"."NO = ".$sum1[$pc][2]."<br>(".$prc2[$pc].")%<br><br>"."NOT SET = ".$sum1[$pc][0]."<br>(".$prc0[$pc].")%";
                }
                // 20 LAUNCH IN JEOPARDY
                if ($pc == 20){
                    if ($xpdp[$tp][$pc] == "YES"){
                        $sum1[$pc][1] = $sum1[$pc][1] + 1;
                    } 
                ...
                // 22, 23 UAT and RA TESTED
                if ($pc == 22){
                    switch ($xpdp[$tp][22]) {
                       case ($xpdp[$tp][22] == "YES" && $xpdp[$tp][23] == "YES"):
                             $sum1[22][1] = $sum1[22][1] + 1; 
                             if ($xpdp[$tp][21] > 0){
                                 $sum1x[22][1] = $sum1x[$pc][1] + $xpdp[$tp][21];
                             } else {
                                 $sumz[$pc][1] = 0;
                                 $sumy[$pc][1] = 0;
                             }
                             break;
                       case ($xpdp[$tp][22] == "YES" && $xpdp[$tp][23] == "NO"): 
                             $sum1[22][2] = $sum1[22][2] + 1; 
                             if ($xpdp[$tp][21] > 0){
                                 $sum1x[22][2] = $sum1x[22][2] + $xpdp[$tp][21];
                             } else {
                                 $sumz[$pc][2] = 0;
                                 $sumy[$pc][2] = 0;
                             }
                             break;
                       case ($xpdp[$tp][22] == "NO" && $xpdp[$tp][23] == "YES"): 
                             $sum1[22][3] = $sum1[22][3] + 1; 
                             if ($xpdp[$tp][21] > 0){
                                 $sum1x[22][3] = $sum1x[22][3] + $xpdp[$tp][21];
                             } else {
                                 $sumz[$pc][3] = 0;
                                 $sumy[$pc][3] = 0;
                             }
                             break;
                       case ($xpdp[$tp][22] == "NO" && $xpdp[$tp][23] == "NO"): 
                             $sum1[22][4] = $sum1[22][4] + 1; 
                             if ($xpdp[$tp][21] > 0){
                                 $sum1x[22][4] = $sum1x[22][4] + $xpdp[$tp][21];
                             } else {
                                 $sumz[$pc][4] = 0;
                                 $sumy[$pc][4] = 0;
                             }
                    ...
                    if ($tp == $rowcount){
                        for ($x=1;$x<=4;++$x) {
                             if ($sum1[22][$x] == 0){
                                 ...
                                 (float)$sumz[22][$x] = round(($sum1[22][$x]/$ypdp)*100,2);
                             }
                             if ($sum1x[22][$x] == 0){
                                 $sumy[22][$x] = 0;
                             } else {
                                 (float)$sumy[22][$x] = round(($sum1x[22][$x]/$yisu)*100,2);
                             }
                        }    
                    }
                    //Analysis
                    $analsis[22] = "<u>UAT=YES,RA=YES<br></u><br>PDP<br>".$sum1[22][1]." out of ".$ypdp."<br>(".$sumz[22][1].")%<br><br>ISSUES<br>".$sum1x[22][1]." out of ".$yisu."<br>(".$sumy[22][1].")%<br><br>".
                                   "<u><br>UAT=YES,RA=NO<br></u><br>PDP<br>".$sum1[22][2]." out of ".$ypdp."<br>(".$sumz[22][2].")%<br><br>ISSUES<br>".$sum1x[22][2]." out of ".$yisu."<br>(".$sumy[22][2].")%<br><br>".
                                   "<u><br>UAT=NO,RA=YES<br></u><br>PDP<br>".$sum1[22][3]." out of ".$ypdp."<br>(".$sumz[22][3].")%<br><br>ISSUES<br>".$sum1x[22][3]." out of ".$yisu."<br>(".$sumy[22][3].")%<br><br>".
                                   "<u><br>UAT=NO,RA=NO<br></u><br>PDP<br>".$sum1[22][4]." out of ".$ypdp."<br>(".$sumz[22][4].")%<br><br>ISSUES<br>".$sum1x[22][4]." out of ".$yisu."<br>(".$sumy[22][4].")%";
                }
         }
   } 
   ...
   for ($tp=1;$tp<=$rowcount;++$tp) {
		...
		//
		// The above line has been altered from $querypae so that both iteration counts can be counted.
		// ===================================================
		// ===================================================
        //a.etl_id = '$yetl_id'               
        //print($querypdp2."<br>");                
        ...
        while($rowpae  = mysql_fetch_row($mysql_datapae)) {
              ...
              for ($pc=$paestart;$pc<=$paeend;++$pc) {
                   ...
                   if ($tp == 1){
                       ...
                       if ($pc == $paestart || $pc2 == 2  || $pc2 == 3 || $pc2 == 4 || $pc2 == 7  || $pc2 == 8  || $pc2 == 9  || $pc2 == 10  || $pc2 == 11  || $pc2 == 12  || $pc2 == 13 || $pc2 == 14 || $pc2 == 15 || $pc2 == 16 || $pc2 == 17  ){
                   ...
                   if ($pc2 == 7 || $pc2 == 8 || $pc2 == 12 || $pc2 == 13){
                       ...
                       if ($pc2 == 5){
                           //if ($pae[$tp][$pc] > 0){
                               $xy = $pc-2;    // Department
                               $xz = $pc-1;    // Iteration
                               $sum2head[$pc] = $pae[$tp][$xy]."<br><br>".$pae[$tp][$xz]; //."<br><br>".$pae[$tp][$xz]; ---> Took Iteration out of analysis display
        ...
        while($rowpwe  = mysql_fetch_row($mysql_datapwe)) {
              ...
              for ($pc=$pwestart;$pc<=$pweend;++$pc) {
                   ...
                   if ($tp == 1){
                       ...
                       if ($pc == $pwestart || $pc2 == 2  || $pc2 == 3  || $pc2 == 4 || $pc2 == 6 || $pc2 == 7){
                           $pwe_head_use[$pc] = 0;
                           //if ($pc <> 0){
                               $pc3c = $pc3c + 1;
                   ...
                   if ($pc2 == 6 || $pc2 == 7){
                           ...
                           //$sum3y[$pc]    = round(($sum3[$pc]/$ytp[$pc]),2);
                           $analsis3[$pc] =  "<u>".$sum3head[$pc]."</u>"."<br><br>".$ytp[$pc]." out of ".$sum1[1]."<br><br>".$sum3y[$pc];
                       //}
                   } else {
                       //$pwe[$tp][$pc] = stripslashes($rowpwe[$pc2]);
                       if ($pc2 == 4 || $pc2 == 5){
   ...
   $querymax      = " select max(etl_id) from ".$name.".tgt_report_groups ";
   $mysql_datamax = mysql_query($querymax, $mysql_link) or die ("#querymax Could not query: ".mysql_error());
   while($rowmax   = mysql_fetch_row($mysql_datamax)) {
         $etlmaxid = stripslashes($rowmax[0]);
   }
   
   $query2a       = "  select issue_type,issue_class,report_group
                         from ".$name.".tgt_report_groups where etl_id = '$etlmaxid' ";
   //where etl_id = '$yetl_id'                      
   $mysql_data2a  = mysql_query($query2a, $mysql_link) or die ("#query2a Could not query: ".mysql_error());       
   $isucnt = 0;
   while($row2a   = mysql_fetch_row($mysql_data2a)) {
   ...
   // ============================ PDP AND CRITERIA ===========================
   //$pagelink = "http://" . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];             //getenv('REQUEST_URI');
   $captnx = "Criteria";
   print("
                        <div id=\"One\" class=\"cont\"> 
                         <table>
                          ...
                          <tr>
                           <th bgcolor=\"#99CCFF\" align=\"right\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:200px;\">
                            <font color=\"#330099\">Build Paperwork</font>
                           </th>
                           <th bgcolor=\"#FFFFCC\" align=\"left\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:300px;\">
                            <font color=\"#330099\">$bld_pw_ind</font>
                           </th>
                          </tr>
                          <tr>
                           <th bgcolor=\"#99CCFF\" align=\"right\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:200px;\">
                          ...
                          <tr>
                           ...
                           <th bgcolor=\"#FFFFCC\" align=\"left\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:300px;\">
                            <font color=\"#330099\">SUMMARY REPORT</font>
                           </th>
                          </tr>                                         
                          <tr>
                           <th bgcolor=\"#99CCFF\" align=\"right\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:200px;\">
                          ...
                          <!--<tr>
                           <th bgcolor=\"#99CCFF\" align=\"right\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:200px;\">
                            <font color=\"#330099\">PAGE LINK</font>
                           </th>
                           <th bgcolor=\"#FFFFCC\" align=\"left\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:300px;\">
                            <font color=\"#330099\">$pagelink</font>
                           </th>
                          </tr>-->
   ");
   if ($total_pdp == 0){
   ...
   ");//ISSUES ROOT CAUSE SUMMARY $etlmaxid
   //////////////////////////////////////////////////////////////////
   /////////////////////////////////////////////////////////////////  
   //tgt_pdp_main
   ...
   for ($pc=1;$pc<=$pcol;++$pc) {
        if ($pdp_head_use[$pc] == 1) {
            if ($pc == 3 || ($pc >= 10 && $pc <=19) || ($pc >= 21 && $pc <=23)){ 
                $wdth = "150px";
            } else {
                $wdth = "75px"; 
            }
   ...
   for ($pc=1;$pc<=$paeend;++$pc) {
        ...
        if ($pae_head_use[$pc] == 1) {
            $wdth = "75px";
            if ($bpc == 5 || $bpc == 6 || $bpc >= 9 && $bpc <= 17 || $pc >= $paeend-2){
                print("<th bgcolor=\"$bcolr\" align=\"center\" valign=\"top\" style=\"width: $wdth;\">
                        <font color=\"$fcolr\">
                         $analsis2[$pc]
                        </font>
   ...
   //////////////////////////////////////////////////////////////////
   /////////////////////////////////////////////////////////////////
   function convert_to_hrmins($analsis3hrs){
       // used to convert hrs fraction to hh:mm
       // returns hh:mm
       	         $y_analsis3hrs   = intval($analsis3hrs);                       //derives hours 
             $y_analsis3mins  = $analsis3hrs - $y_analsis3hrs;              //derives fraction that represents minutes
		     $u_analsis3mins  = round(($y_analsis3mins*60),0);
		     $la_analsis3hrs  = strlen($y_analsis3hrs);
		     $la_analsis3mins = 2 - strlen($u_analsis3mins);
		     if ($la_analsis3hrs   == 0){
		         $lb_analsis3hrs   = $y_analsis3hrs ;
		     } else {
		       $lb_analsis3hrs  = $y_analsis3hrs ;
             }
		     if ($la_analsis3mins == 0){
		         $lb_analsis3mins = $u_analsis3mins;
		     } else {
               if ($la_analsis3mins == 1){
                   $lb_analsis3mins = "0". $u_analsis3mins; 
               }
               if ($la_analsis3mins == 2){
                   $lb_analsis3mins = "00";
               }
             }
             $lytdt_hrs  =   $lb_analsis3hrs ;
             $lytdt_mins =   $lb_analsis3mins;
             
              return "$lytdt_hrs : $lytdt_mins";
   ...
   for ($pc=1;$pc<=$pweend;++$pc) {
        ...
        if ($pwe_head_use[$pc] == 1) {
            ...
            if ($bpc >= 4 && $bpc <= 7){ 
                if ($bpc == 6 || $bpc == 7){
                     $bpcx = $pc - 2;
				  //$analsis3hrs = intval($analsis3[$bpcx]/60);
				  $analsis3hrs = $analsis3[$bpcx];
	         ///////////////////////////////////////
                     $res_fr_to_hrsmin = convert_to_hrmins($analsis3hrs);	 
	         ///////////////////////////////////////
		     //$ytd_tot_rwrk_hrs[$h]
				  // Replaced $analsis3hrs in place of $analsis3[$bpcx]
                  print("<th bgcolor=\"$bcolr\" align=\"center\" valign=\"top\" style=\"width: $wdth;\">
                          <font color=\"$fcolr\">
                           $analsis3[$pc]%<br><br>$lytdt_hrs : $lytdt_mins<br>Hrs : Mins
                          </font>
                         </th>        
                  ");
                } else {
	         ...
	         $res_fr_to_hrsmin = convert_to_hrmins($analsis3hrs);
                 ///////////////////////////////////////
                  print("<th bgcolor=\"$bcolr\" align=\"center\" valign=\"top\" style=\"width: $wdth;\">
                          <font color=\"$fcolr\">
                           " . $analsis3[$pc+2] . "%<br><br>$res_fr_to_hrsmin<br>Hrs : Mins
                          </font>
                         </th>        
                  ");
                }
    ...
    //if ($xpdp[$tp][$pcol] == 1){
        ...
        for ($pc=1;$pc<=$pcol;++$pc) {
             if ($pdp_head_use[$pc] == 1) { 
				 ...
				 $val1 = $xpdp[$tp][10];
				 $val2 = $xpdp[$tp][22];
				 if ($val1 == "IN SCOPE" && $val2 == "NO"){
				            if ($pc == 1){
                                $bcolr = "#FF0000";
                                $fcolr = "#FFFFFF";
							}
				 }
				 if ($val1 == "NOT SET" && $val2 == "YES"){
				            if ($pc == 1){
                                $bcolr = "#FF0000";
                                $fcolr = "#FFFFFF";
							}
				 }
				 if ($val1 == "OUT OF SCOPE" && $val2 == "YES"){
				            if ($pc == 1){
                                $bcolr = "#FF0000";
                                $fcolr = "#FFFFFF";
							}
        ...
        for ($pc=1;$pc<=$pweend;++$pc) {
             ...
             if ($pwe_head_use[$pc] == 1) {
                 ...
                 if ($bpc == 6 || $bpc == 7){
                     ...
                     $val = convert_to_hrmins($pwe[$tp][$pc]);
                 } else {
                     $val = $pwe[$tp][$pc];
                 }
   ...
   // Details End 
   // ============= 
   $xfilters = "Type = ".$prd."<br>REVENUE ASSURANCE TESTING = ".$ra_ind."<br>PDP CATEGORY = ".$pdpcat."<br>BUILD PAPERWORK = ".$bld_pw_ind."<br>LAUNCH DATE BETWEEN ".$xldate." AND ".$xldate2;
   $xreport_name = "SUMMARY_REPORT"; 
   ...
   //$scrpt = "/report_summary_report.php";
   //$scrptlen = strlen($scrpt);
   $loc  =  $_SERVER['REQUEST_URI'];
   //print($loc."<br>");
   $locx = substr($loc,0,-26);
   //print($locx."<br>");   
   ...
   $f = fopen($xreport_path, "c+") or die ("could not open file.");          //fopen($xreport_path, "w+")
...
if ($found == 0) {
    ...
    $ind_id[1]   = 1;
    $ind_id[2]   = 0;
    $ind_id[3]   = 0; // this is not in fact being used
    ...
    print("<form method=\"post\" action=\"./report_summary_report.php\">
    ...
    print(" 
            ...
            <tr>
             <td bgcolor=\"#99CCFF\" align=\"right\" style=\"width:150px;\">
              <font color=\"#330099\">Select Build Paperwork:</font>
             </td>
             <td> 
              <select name=\"bld_pw_ind\">
    ");
    $e = 0;
    for ($e=0;$e<=3; ++$e) {
         print(" <option value=\"$ind[$e]\">$ind[$e]</option> "); 
    }      
    // ******************** Build (END) - Launch Date (START) ****************************
    print("
              </select>
             </td>
            </tr> 
   ...
   $query = "  select saved_report_id,report_name,filters,report_path,etl_id,report_timestamp,ran_by from ".$name.".saved_reports
                where rtrim(report_name) = 'SUMMARY_REPORT'
                  and ran_by = '$usr'
