...
//print($end_d." - ".$end_m." - ".$end_y."<br>");
   ...
   if ($rowcntx > 0) {
       ...
       while($rowx = mysql_fetch_row($mysql_datax)) {
             ...
             $xbuild_pw = "";         
             $ybuild_pw  = stripslashes($rowx[18]);
			 if ($ybuild_pw == 1){
			     $xbuild_pw = "YES";
			 }
			 if ($ybuild_pw == 2){
                 ...
                 for ($d=1;$d<=$dcnt;++$d) {
                  ...
                  for ($i=1;$i<=$icnt;++$i) {
                      ...
                      if ($xdpti == 0) {
                              ...
                              $query105 = "INSERT into ".$name.".tgt_pdp_issue_area_summary(etl_id,pdp,issue_area,found_issues,issue_type,issue_class,report_group,group_used)
                                           VALUES('$xetl_id','$xpdp_desc','$dpt[$d]','$xdpti','$ityp[$i]','$iccd[$i]','$irgrp[$i]','$igrp[$i]')";
                              print($query105."<br>");             
                              $mysql_data105 = mysql_query($query105, $mysql_link) or die ("#23.5 Could not query: ".mysql_error());
                      }
                  }
...
//print("This will not be displayed due to the above error.");
       ...
       for ($i=1;$i<=$icnt;++$i) {
            if ($iccd[$i] == "ROT"){
                ...
                $query402 = "INSERT into ".$name.".tgt_root_cause_ytd(etl_id,issue_type,ytd_count,etl_year,issue_class,report_group,group_used)
                             VALUES('$xetl_id','$ityp[$i]',$ytd_occurance,$etl_year,'$iccd[$i]','$irgrp[$i]','$igrp[$i]')";
                print($query402."<br>");             
       ...
       for ($d=1;$d<=$dcnt;++$d) {
	            if ($yeartoday == $etl_year){
                       $query397 = "SELECT issue_area,sum(back_to_build),sum(if(back_to_build = 0, 0 , 1))
                ...
                if ($rowcnt397 == 0){
                        ...
                        $ytd_back2build = 0;
                        $ytd_back2build_pdp_cnt = 0;
                } else { 
                  while($row397 = mysql_fetch_row($mysql_data397)) {
                        $ytd_issue_area = stripslashes($row397[0]);
                        $ytd_back2build = stripslashes($row397[1]);
                        $ytd_back2build_pdp_cnt = stripslashes($row397[2]);
                  }  
                }                       
            $query403 = "INSERT into ".$name.".tgt_back_to_build_ytd(etl_id,area,ytd_count,etl_year, ytd_pdp_cnt)
                         VALUES('$xetl_id','$dpt[$d]',$ytd_back2build,$etl_year, $ytd_back2build_pdp_cnt)";
            print($query403."<br>");             
       ...
       for ($d=1;$d<=$dcnt;++$d) {
	            if ($yeartoday == $etl_year){
                        $query396 = "SELECT issue_area,sum(baseline_hours),sum(rework_hours), sum(if(rework_hours > 0, 1, 0))
                ...
                if ($rowcnt396 == 0){
                  ...
                  while($row396 = mysql_fetch_row($mysql_data396)) {
                        ...
                        $ytd_rework_hrs_pdp_cnt = stripslashes($row396[3]);
                  }  
                }                       
            $query404 = "INSERT into ".$name.".tgt_rework_hours_ytd(etl_id,area,ytd_count_base,ytd_count_rework,etl_year,ytd_rework_hrs_pdp_cnt)
                         VALUES('$xetl_id','$dpt[$d]',$ytd_baseline_hrs,$ytd_rework_hrs,$etl_year, $ytd_rework_hrs_pdp_cnt)";
            print($query404."<br>");             
...
if ($found == 0) {
	...
	for ($e=$currentyear;$e>=2010;$e=$e-1) {
