...
if (isset($new)) {
	while (list($key) = each($new)) {
            ...
            $query ="INSERT into ".$name.".cubes(cube_name,cube_ind)
           ...
           <form method=\"post\" action=\"./cubeform.php\">
            <table border='0' align=\"center\" scroll=\"yes\">
             ...
             <tr> 
               ...
               <td align=\"center\" bgcolor=\"#99CCFF\"><font color=\"#330099\">Cubes</font></td>
...
$query = "select cube_id,cube_name,cube_ind from ".$name.".cubes";
$mysql_data = mysql_query($query, $mysql_link) or die ("Could not query: ".mysql_error());
...
while($row = mysql_fetch_row($mysql_data)) {
    ...
    $xcube_name      = stripslashes($row[1]);
    $xcube_ind       = stripslashes($row[2]);    
