...
if ($start == 1) {
//print($kiid."<br>");
//print($xds." - ".$xms." - ".$xys."<br>".$xde." - ".$xme." - ".$xye."<br>");
$xs_dt = mktime(0,0,0,$xms,$xds,$xys);
$xe_dt = mktime(0,0,0,$xme,$xde,$xye);
//print($xs_dt."<br>".$xe_dt);
// ============================== ISSUE TYPE START ==============================
$query69               = "select a.issue_type_id,a.issue_type,a.issue_type_ind,b.issue_class_code,
                                 a.issue_seq,a.report_group_id,a.parent_issue_type_id,a.issue_type_ind
...
$icnt                  = 0;
$rcount                = 0;  // Count ROT and GRT
$ricount               = 0;  // Count how many inactive in ROT
$gicount               = 0;  // Count how many inactive in GRT
$mcount                = 0;  // Count CNT
$micount               = 0;  // Count how many inactive in CNT
$mrcount               = 0;  // Count how many have report_group_id = 1 in CNT
while($row69           = mysql_fetch_row($mysql_data69)) {
      ...
      $ind[$icnt]      = stripslashes($row69[7]);        // 0 for inactive, 1 for active
      $igrp[$icnt]     = 0;
      $mcounbypass     = 0;                             //
      if ($irgrpid[$icnt] == 0) {
      ...
      if ($iccd[$icnt] == "CNT"){
        $mcount = $mcount + 1;
        if ($ind[$icnt] == 0){
            $micount = $micount + 1;                 // count if issue_type_ind = 0 i.e. Active
        }
        if ($irgrpid[$icnt] == 1){
            $mrcount = $mrcount + 1;                 // count if report_group_id = 1
        }
        ...
        while($row96        = mysql_fetch_row($mysql_data96)) {
              ...
              if ($ifound == 0){
                  //print($iccd[$icnt]." # ".$icnt." - issue_type_id - ".$iid[$icnt]." - issue_type - ".$ityp[$icnt]." - report_group - ".$irgrp[$icnt]." - group_used - ".$igrp[$icnt]." - parent_issue_type - ".$ipid[$icnt]." - issue_type_ind - ".$ind[$icnt]."<br>");
                  $xiid      =  $iid[$icnt];
                  $xityp     =  $ityp[$icnt];
      ...
      if ($iccd[$icnt] == "ROT"){
          ...
          while($row77           = mysql_fetch_row($mysql_data77)) {
                ...
                $ind[$icnt]      = stripslashes($row77[7]);        // 0 for inactive, 1 for active
                if ($ind[$icnt] == 0){
                    $gicount = $gicount + 1;
                }
                $irgrp[$icnt]    = $irgrp[$icnt-1];
...
print("<html>
        ...
        <body>
         <table border='1' align=\"center\" width=\"100%\" style=\"border: 1px solid #CCCCCC; border-color:#CCCCCC; background-color: #FFFFFF\">
              <tr>
               <td bgcolor=\"#E8E8E8\" colspan=\"12\" rowspan=\"2\" align=\"center\" valign=\"top\" style=\"width: 50px;\">
                <font color=\"#330099\">
                 &nbsp
                </font>
               </td>               
               <td bgcolor=\"#99FF99\" colspan=\"$rcount\" align=\"center\" valign=\"top\">
                <font color=\"#330099\">
                 ROOT CAUSE
                </font>
               </td>  
               <td bgcolor=\"#9AFEFF\" colspan=\"$mcount\" rowspan=\"2\" align=\"center\" valign=\"top\">
                <font color=\"#330099\">
              ...
              <tr>
               <td bgcolor=\"#FFFF00\" colspan=\"$rcount\" align=\"center\" valign=\"top\">
                <font color=\"#330099\">
                 GRANULAR ROOT CAUSE
                </font>
               </td>
              ...
              <tr>
               <td bgcolor=\"#CCFFCC\" align=\"center\" valign=\"top\" style=\"width: 100px;\">
                <font color=\"#330099\">
                 PDP
                </font>
               </td>
               ...
               <td bgcolor=\"#CCFFCC\" align=\"center\" valign=\"top\" style=\"width: 50px;\">
                <font color=\"#330099\">
                 Issue<br>Count
                </font>
               </td>               
               ...
               <!--<td bgcolor=\"#CCFFCC\" align=\"center\" valign=\"top\" style=\"width: 150px;\">
                <font color=\"#330099\">
                 Issues and Updates
                </font>
               </td>-->
               ...
               <td bgcolor=\"#CCFFCC\" align=\"center\" valign=\"top\" style=\"width: 75px;\">
                <font color=\"#330099\">
                 Raised By
                </font>
               </td>                                             
               <td bgcolor=\"#99FF99\" align=\"center\" valign=\"top\" style=\"width: 150px;\">
                <strong><font color=\"#330099\">
                 Root<br>Cause
                </font></strong>
               </td>
               <td bgcolor=\"#9AFEFF\" align=\"center\" valign=\"top\" style=\"width: 150px;\">
                <strong><font color=\"#330099\">
                 Missed<br>Opportunities
                </font></strong>
               </td>
");
for ($i=1;$i<=$icnt ; ++$i) {
     if ($iccd[$i] == "ROT" or $iccd[$i] == "GRT") {
         if ($iccd[$i] == "ROT"){
             $colr = "#99FF99";
         }
         if ($iccd[$i] == "GRT"){
     ...
     if ($ind[$i] == 0){
         ...
         print(" <td align=\"center\" valign=\"top\" style=\"width: 50px; background-color: $colr;\">
                  <font color=\"$fcolr\">
                   $ityp[$i]
                  </font>
                 </td> 
...
");
	   ...
	   if ($pdp <> "ALL") {
           ...
           $query78       = "select a.pdp_id,a.pdp_desc,a.updated_date,a.updated_by,a.pdp_name,a.pdp_owner,a.pdp_launch,
                                    ...
                                    a.revenue_id,a.comparison_id,a.pdp_period_id,a.main_pdp_id,a.parent_pdp_id 
                               from ".$name.".pdp a
                              where pdp_launch >= '$xs_dt' 
                                and pdp_launch <= '$xe_dt' 						 
                           order by a.pdp_launch";
	   }
       //$query78       = "select a.pdp_id,a.pdp_desc,a.updated_date,a.updated_by,a.pdp_name,a.pdp_owner,a.pdp_launch,
       //                        a.pdp_status,a.pdp_period_id,a.pdp_category_id,a.complexity_id,a.projection_id,
       //                        a.revenue_id,a.comparison_id,a.pdp_period_id,a.main_pdp_id,a.parent_pdp_id 
       ...
       //               order by a.pdp_launch";
	   //				  //$xpdp_launch_dt
	   //				  //a.pdp_desc like '2011-%'
       //print($query78."<br>");            
       $mysql_data78 = mysql_query($query78, $mysql_link) or die ("#78 - Could not query: ".mysql_error());
       ...
       while($row80  = mysql_fetch_row($mysql_data80)) {
			 ...
			 $piignore[$picnt] = 0; // Indicator to control display of issues
       ...
       for ($i=1;$i<=$icnt ; ++$i) {
            //if ((($iccd[$i] == "ROT") && ($igrp[$i] == 0)) or (($iccd[$i] == "GRT") && ($igrp[$i] == 0))) {
            if ($iccd[$i] == "ROT") {
       ...
       if ($rowcnt80 <> 0) {
        //print ("<tr> 
        //         <td bgcolor=\"#FFFFFF\" colspan=\"9\" align=\"center\" valign=\"middle\">
        //          <font color=\"#330099\">
        //           &nbsp
        //          </font>
        //         </td>
        ...
        //       </tr>  
        //"); 
		// Taking GROUPS LINE OUT, BASED ON REQUEST FROM WENDY KERR
		// --------------------------------------------------------------------------------------
        //print ("<tr> 
        //         <td bgcolor=\"#FFFFFF\" colspan=\"7\" align=\"center\" valign=\"middle\">
        //          <font color=\"#330099\">
        //           &nbsp
        //          </font>
        //         </td>
        ...
        //         } 
        //         print("
        //                <td bgcolor=\"$colr\" align=\"center\" valign=\"middle\">
        //                 <font color=\"$fcolr\">
        //                  $prtotlr%
        //                 </font>
        //                </td> 
        ...
        for ($p=1;$p<=$picnt ; ++$p) {
		 if ($piignore[$p] == 1){
           $totisu = $totisu + 1;
  		   $v_issue_id = $piid[$p];
           if ($pimax[$p] == 1){
             if (trim($pidpt[$p]) == "MARKETING OPERATIONS"){
                      $dbcolr = "#ECE5B6";  
             }
             if (trim($pidpt[$p]) == "REVENUE ASSURANCE"){
                      $dbcolr = "#FFF8C6";  
             }
             if (trim($pidpt[$p]) == "ENTERPRISE UAT"){
                      $dbcolr = "#CCFFCC";  
             }
             if (trim($pidpt[$p]) == "CONFIGURATION MANAGEMENT"){
                      $dbcolr = "#E3E4FA";  
             }
             print("<tr>
                     <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 100px;\">
                      <font color=\"#330099\">
                       $xpdp_desc
                      </font>
                     </td>                    
                     ...
                     <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 50px;\">
                      <font color=\"#330099\">
                       $p
                      </font>
                     </td>
                     <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 50px;\">
                      <font color=\"#330099\">
                       <!--<a href=\"javascript:void(0);\" onclick=\"PopupCenter('./pdp_issue_details.php?v_issue_id=$v_issue_id', 'myPop1',1000,800);\">$piid[$p]</a>-->
                       <a href=\"javascript:void(0);\" onclick=\"popup2('./pdp_issue_details.php?v_issue_id=$v_issue_id&&pdp=$xpdp_desc&&pdptype=$xpdp_period')\">$piid[$p]</a>
                      </font>
                     </td>
                     <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 50px;\">
                       <font color=\"#330099\">
            ...
            for ($l=1;$l<=$pimax[$p] ; ++$l) {
                  ...
                  if ($l <> 1 && $l == $pimax[$p]) {
                      ...
                      $uon    = $pudt[$p][$l];
                      $udpt   = $pudpt[$p][$l];
                      if (trim($udpt) == "MARKETING OPERATIONS"){
                               $dbcolr = "#ECE5B6";  
                      }
                      if (trim($udpt) == "REVENUE ASSURANCE"){
                               $dbcolr = "#FFF8C6";  
                      }
                      if (trim($udpt) == "ENTERPRISE UAT"){
                               $dbcolr = "#CCFFCC";  
                      }
                      if (trim($udpt) == "CONFIGURATION MANAGEMENT"){
                               $dbcolr = "#E3E4FA";  
                      }
                      print("<tr>
                              <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 100px; border: 1px solid #CCCCCC;\">
                               <font color=\"#330099\">
                                $xpdp_desc
                               </font>
                              </td>                      
                              ...
                              <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 50px; border: 1px solid #CCCCCC;\">
                               <font color=\"#330099\">
                                $p
                               </font>
                              </td>
                              <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 50px; border: 1px solid #CCCCCC;\">
                               <font color=\"#330099\">
                                <!--<a href=\"javascript:void(0);\" onclick=\"PopupCenter('display_issue_details2.php?v_issue_id=$v_issue_id', 'myPop1',1000,800);\">$piid[$p]</a>-->
                                <a href=\"javascript:void(0);\" onclick=\"popup2('./pdp_issue_details.php?v_issue_id=$v_issue_id&&pdp=$xpdp_desc&&pdptype=$xpdp_period')\">$piid[$p]</a>
                               </font>
                              <td bgcolor=\"$dbcolr\" align=\"center\" valign=\"middle\" style=\"width: 50px; border: 1px solid #CCCCCC;\">
                               <font color=\"#330099\">
                              ...
                              <!--</td>
                               <td bgcolor=\"#CCFFCC\" align=\"center\" valign=\"middle\"  valign=\"middle\" style=\"width: 150px; border: 1px solid #CCCCCC;\">
                               <font color=\"#330099\">
                                $uudesc
                               </font>
                              </td>-->                                        
                ...
                //}
                  ...
                  if ($l <> $pimax[$p]) {
                    ...
                    if (($parityrot > 0 && $paritycnt == 0) && $parityind == 1) {
                         ...
                         if ($parityrot > 1){
                             $paritychk = $paritychk.", AND MULTIPLE ROOT CAUSES";
                         }
                    }
                    if (($parityrot == 0 && $paritycnt > 0) && $parityind == 1) {
         ...
         $wdth = 10 + $icnt;          
         //print ("<tr> 
         //         <td bgcolor=\"#FF0000\" colspan=\"$wdth\" align=\"left\" valign=\"middle\">
         //          <font color=\"#FFFFFF\">
         //           NO ISSUES TO REPORT
         //          </font>
         //          </td>
       ...
       // =============================================================================================
       $xstotlr = $xstotlr + $stotlr;
       $xstotlc = $xstotlc + $stotlc;
       for ($i=1;$i<=$icnt; ++$i) {             // loop through totals 
            $xtcnt[$i] = $xtcnt[$i] + $tcnt[$i];
       }	   
       unset($tcnt,$piid,$pidesc,$pitid,$pityp,$pimax,$puid,$pudesc,$putid,$putyp,$pi,$vval,$ytotl,$stotlr,$stotlc,$prtotlr,$prtotlc,$rtotl,$picby,$picon,$picdt,$pucby,$pucon,$pudt,$pidpt,$pudpt);
       //print(" <!--</tr>
       //       </table>-->
       //");       
       }
       print ("
                <tr> 
                 ...
                 <td bgcolor=\"RED\" align=\"center\" valign=\"middle\">
                  <font color=\"#FFFFFF\">
                   Net Total
                  </font>
                 </td>
       ...
       print("
			  ...
			  <tr>
               <td bgcolor=\"#FFFFFF\" colspan=\"12\" align=\"center\" valign=\"middle\">
                  <font color=\"#330099\">
                   &nbsp
                  </font>
               </td> 
...
// ============================== ISSUE TYPE START ==============================
$query69               = "select a.issue_type_id,a.issue_type,a.issue_type_ind,b.issue_class_code,
                                 a.issue_seq,a.report_group_id,a.parent_issue_type_id,a.issue_type_ind,b.issue_class
                            from ".$name.".issue_types a, ".$name.".issue_class b
                           where a.issue_class_id = b.issue_class_id
                             and b.issue_class_code <> 'GRT'
                        order by b.issue_class_code desc,a.issue_seq asc";
...
while($row69           = mysql_fetch_row($mysql_data69)) {
      ...
      $ityp_ind[$icnt] = stripslashes($row69[2]);
      $iccd[$icnt]     = trim(stripslashes($row69[3]));  //issue_class_code
      $icc[$icnt]      = trim(stripslashes($row69[8]));  //issue_class
      //$iseq[$icnt]   = stripslashes($row69[4]);
      $irgrpid[$icnt]  = stripslashes($row69[5]);        //report_group_id
      $ipid[$icnt]     = stripslashes($row69[6]); 
      $ind[$icnt]      = stripslashes($row69[7]);        // 0 for inactive, 1 for active
      ...
      if ($iccd[$icnt] == "ROT"){
          ...
          while($row77           = mysql_fetch_row($mysql_data77)) {
                ...
                $ityp_ind[$icnt] = stripslashes($row77[2]);
                $iccd[$icnt]     = trim(stripslashes($row77[3]));  //issue_class_code
				$icc[$icnt]      = trim(stripslashes($row77[8]));  //issue_class
                //$iseq[$icnt]   = stripslashes($row69[4]);
                $irgrpid[$icnt]  = stripslashes($row77[5]);        //report_group_id
                $ipid[$icnt]     = stripslashes($row77[6]);
                $ind[$icnt]      = stripslashes($row77[7]);        // 0 for inactive, 1 for active
...
for ($i=1;$i<=$icnt ; ++$i) {
      //print("ID: ".$iid[$i]." - TYPE: ".$ityp[$i]." - RGROUP: ".$irgrp[$i]." - GROUP ID: ".$irgrpid[$i]." - CLASS: ".$icc[$i]." - PARENT: ".$ipid[$i]." - CLASS CODE: ".$iccd[$i]."<br>");
}
// ============================== ISSUE TYPE END ==============================

    // ==============================
    $querys66 = "SELECT issue_area,issue_area_id FROM ".$name.".issue_areas
    ...
    while ($row66 = mysql_fetch_row($mysql_data66)) {
           $parmtrcnt = $parmtrcnt + 1;
           $parm[$parmtrcnt]     = stripslashes($row66[0]);
		   $parm_id[$parmtrcnt]  = stripslashes($row66[1]);
    }
	$parmtrcnt = $parmtrcnt + 1;
    $parm[$parmtrcnt]    = "ALL";
	$parm_id[$parmtrcnt] = 0;
    ...
    // ==============================
    //$captn = "Select Department";
    print("<body>
            <form method=\"post\" action=\"./pdp_issues_by_dept.php\">
             <table border='0' scroll=\"yes\">
              <!--<caption>$captn</caption>-->
              <tr>
               <td bgcolor=\"#99CCFF\" align=\"right\" style=\"width:150px;\">
                <font color=\"#330099\">Select Department:</font>
               </td>
               <td> 
                <select name=\"parmtr\">
    ");
    ...
    for ($u=1;$u<=$parmtrcnt;++$u) {
        print("<option value=\"$parm_id[$u]\">$parm[$u]</option>");
    }     
    print("
              </select>
             </td>
    ...
    print(" 
            <tr>
             <td bgcolor=\"#99CCFF\" align=\"right\" style=\"width:150px;\"><font color=\"#330099\">FROM - Launch Date (dd-mm-yyyy)</font></td>
             <td align=\"left\" valign=\"middle\" bgcolor=\"#FFFFFF\" style=\"width:300px;\">
              <font color=\"#330099\">
              <select align=\"left\" name=\"xds\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
	");
    for ($xdy_s=1;$xdy_s<=31; ++$xdy_s) {
         print(" <option value=\"$xdy_s\">$xdy_s</option> ");
    }
    print(" </select>
	        <select align=\"left\" name=\"xms\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
	");
    for ($xmon_s=1;$xmon_s<=12; ++$xmon_s) {
         print(" <option value=\"$xmon_s\">$xmon_s</option> ");
    }
    print(" </select>
	        <select align=\"left\" name=\"xys\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
	");
    ...
    print("    
            ...
            <tr>
             <td bgcolor=\"#99CCFF\" align=\"right\" style=\"width:150px;\"><font color=\"#330099\">TO - Launch Date (dd-mm-yyyy)</font></td>
             <td align=\"left\" valign=\"middle\" bgcolor=\"#FFFFFF\" style=\"width:300px;\">
              <font color=\"#330099\">
              <select align=\"left\" name=\"xde\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
	");
    for ($xdy_s=1;$xdy_s<=31; ++$xdy_s) {
         print(" <option value=\"$xdy_s\">$xdy_s</option> ");
    }
    print(" </select>
	        <select align=\"left\" name=\"xme\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
	");
    for ($xmon_s=1;$xmon_s<=12; ++$xmon_s) {
         print(" <option value=\"$xmon_s\">$xmon_s</option> ");
    }
    print(" </select>
	        <select align=\"left\" name=\"xye\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
	");
    for ($xyr_s=2010;$xyr_s<=2015; ++$xyr_s) {
         print(" <option value=\"$xyr_s\">$xyr_s</option> ");
    }
    print("    
			...
			<tr> 
		     <td bgcolor=\"#99CCFF\" align=\"right\" style=\"width:150px;\"><font color=\"#330099\">ROOT CAUSE</font></td>
			 <td>
              </select>
	           <select align=\"left\" name=\"kiid\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> 
   ...
   print("    </select>
            ...
            <tr>			
             ...
             <td>
              <input type=\"text\" name=\"pdp\" value=\"$pdp\" size=\"9\" maxlength=\"9\">
             </td>
            </tr>
            <tr>
             <td>
