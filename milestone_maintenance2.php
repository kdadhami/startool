    ...
    if (isset($delete)) {
	    while (list($key) = each($delete)) {
               ...
               $query110 = "INSERT into ".$name.".pdp_logs(pdp_id,issue_area,updated_by,updated_on,comments,module,action,followup,phase,phase_desc)
                            VALUES('$pdp_id','$uissue_area','$usr',current_timestamp,'$ycomments','WORK EFFORT',0,1,'WORK EFFORT','$yphase_desc')";
			   //print($query110."<br>");
   ...
   if ($rowcntx == 1) {
      ...
      $captn = "Work Effort Status";
           ...
           while($row44 = mysql_fetch_row($mysql_data44)) {
			    ...
			    ");
                if(($xpdp_launch >= ($new_dt - $twoweeks)) and ($xpdp_launch <= ($new_dt + $twoweeks))){
                  print("				
                       <td align=\"center\" valign=\"middle\" bgcolor=\"$colr1\">
                         <font color=\"#330099\">
