...
//if ($start == 1) {          //// Taken to line 34 above
   ...
   print("
          <!--<div id=\"One\" class=\"cont\">--> 
           <table id=\"theTable4\">
	        <caption>$captnx2</caption>
            <tr class=\"headerRow4\">
             <th bgcolor=\"#CCFFCC\" colspan=\"3\" align=\"center\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:200x;\">
              <font color=\"#330099\">
               (YYYY-MM-DD)<br>$xldate &nbsp&nbspTO&nbsp&nbsp $xldate2
              </font>
             </th>
    ...
    for ($h=$qry0;$h>=1;$h=$h-1) {
     ...
     if ($h == ($qry0-1) or $h == $qry0){     		 
         ...
         print("<th bgcolor=\"$colr\" colspan=\"2\" align=\"center\" valign=\"middle\" style=\"word-wrap: break-word; word-break:break-all; width:200x;\">
                  <font color=\"#330099\">
                   $yr[$h]<br>$dsp_dt_range
                  </font>
                 </th>
    ...
    print("<tr class=\"headerRow4\">
            ...
            <th bgcolor=\"#CCFFCC\" align=\"center\" valign=\"top\" style=\"word-wrap: break-word; word-break:break-all; width:140px;\">
             <font color=\"#330099\">COUNT & AVERAGE</font>
            </th>
            <th bgcolor=\"#CCFFCC\" align=\"center\" valign=\"top\" style=\"word-wrap: break-word; word-break:break-all; width:60px;\">
             <font color=\"#330099\">PDPs</font>
            </th>
   ");
    $clrcnt = 0;
    for ($h=$qry0;$h>=1;$h=$h-1) {
     ...
     if ($h == ($qry0-1) or $h == $qry0){     		 
         ...
         print("<th bgcolor=\"$colr\" align=\"center\" valign=\"top\" style=\"word-wrap: break-word; word-break:break-all; width:140px;\">
                 <font color=\"#330099\">COUNT & AVERAGE</font>
                </th>
                <th bgcolor=\"$colr\" align=\"center\" valign=\"top\" style=\"word-wrap: break-word; word-break:break-all; width:60px;\">
                 <font color=\"#330099\">PDPs</font>
                </th>
         ");
	 }	 
	...
	$tot_b2b_cnt = 0;
        $tot_pdp_cnt = 0;
    for ($r=1;$r<=$qry1;++$r) {
         $query_b2b = " SELECT issue_area,sum(back_to_build),sum(if(back_to_build = 0, 0 , 1))
         ...
         if ($qry_b2b == 0){
			 ...
			 $tot_b2b_cnt  = $tot_b2b_cnt + $area_b2b[$r];
                         $tot_pdp_cnt += $area_pdp[$r];
         } else {		 
           while($rowb2b    = mysql_fetch_row($mysql_b2b)) {
                 $issue_area[$r] = trim(stripslashes($rowb2b[0]));
                 $area_b2b[$r]  = trim(stripslashes($rowb2b[1]));
                 $area_pdp[$r] = trim(stripslashes($rowb2b[2]));
	       	     $tot_b2b_cnt   = $tot_b2b_cnt + $area_b2b[$r];
                     $tot_pdp_cnt += $area_pdp[$r];
           }
		 }
	     for ($x=$qry0;$x>=1;$x=$x-1) {
              $query_ytd = " SELECT etl_id,ytd_count, ytd_pdp_cnt
                               FROM ".$name.".tgt_back_to_build_ytd 
                              WHERE etl_year = '$yr[$x]' 
                                AND area = '$pdparea[$r]' 
                          ";
			  //print($query_ytd."<br>");			  
              $mysql_ytd  = mysql_query($query_ytd, $mysql_link) or die ("#query_ytd Could not query: ".mysql_error() . " <br>Query:" . $query_ytd); 
              $qry_ytd    = mysql_num_rows($mysql_ytd);
              if ($qry_ytd == 0){
                  $ytd_b2b_cnt[$r][$x] = 0;
                  $ytd_b2b_pdp_cnt[$r][$x] = 0;
				  $ytd_tot_b2b_cnt[$x]  = $ytd_tot_b2b_cnt[$x] + $ytd_b2b_cnt[$r][$x];
              }	else {		  
	     	    while($row_ytd = mysql_fetch_row($mysql_ytd)) {
                      $ytd_b2b_cnt[$r][$x] = trim(stripslashes($row_ytd[1]));
                      $ytd_b2b_pdp_cnt[$r][$x] = trim(stripslashes($row_ytd[2]));
	     		      $ytd_tot_b2b_cnt[$x] = $ytd_tot_b2b_cnt[$x] + $ytd_b2b_cnt[$r][$x];
	     	    }
              }			  
	     }
	...
	for ($r=1;$r<=$qry1;++$r) {
	     ...
	     print("<tr>
                 ...
                 <td bgcolor=\"#FFFFFF\" align=\"center\" valign=\"middle\" style=\"width: 140px;\">
                  <font color=\"#330099\">
				   $area_b2b[$r]<br>($b2b_prc[$r]%)
                  </font>
                 </td>
                 <td bgcolor=\"#FFFFFF\" align=\"center\" valign=\"middle\" style=\"width: 60px;\">
                  <font color=\"#330099\">
				   $area_pdp[$r]
                  </font>
                 </td>
    	...
    	for ($h=$qry0;$h>=1;$h=$h-1) {
         ...
         if ($h == ($qry0-1) or $h == $qry0){     		 
		      $x_ytd_b2b_cnt = $ytd_b2b_cnt[$r][$h];
                      $x_ytd_b2b_pdp_cnt = $ytd_b2b_pdp_cnt[$r][$h];
			  if ($x_ytd_b2b_cnt == 0){
			      $x_b2b_prc = 0; 
			  } else {
    ...
    print("<tr class=\"headerRow4\">
            ...
            <th bgcolor=\"#CCFFCC\" align=\"center\" valign=\"middle\" style=\"width: 140px;\">
             <font color=\"#330099\">
      		  <strong>$tot_b2b_cnt<br>(100%)</strong>
             </font>
            </th>
            <th bgcolor=\"#CCFFCC\" align=\"center\" valign=\"middle\" style=\"width: 60px;\">
             <font color=\"#330099\">
      		  <strong>&nbsp;</strong>
             </font>
            </th>
    ...
    $clrcnt = 0;
     ...
     if ($h == ($qry0-1) or $h == $qry0){     		 
		 ...
		 print("
                <th bgcolor=\"$colr\" align=\"center\" valign=\"middle\" style=\"width: 140px;\">
                 <font color=\"#330099\">
                  <strong>$ytd_tot_b2b_cnt[$h]<br>(100%)</strong>
                 </font>
                </th>
                <th bgcolor=\"$colr\" align=\"center\" valign=\"middle\" style=\"width: 60px;\">
                 <font color=\"#330099\">
                  <strong>&nbsp;</strong>
                 </font>
                </th>
   ...
   ");
	...
	$tot_rwrk_hrs = 0;
    for ($r=1;$r<=$qry1;++$r) {
         $query_rwrk = " SELECT issue_area,sum(rework_hours), sum(if(rework_hours > 0, 1, 0))
         ...
         if ($qry_rwrk == 0){
			 ...
			 $tot_rwrk_hrs  = $tot_rwrk_hrs + $area_rwrk[$r];
                         $tot_pdp_rwrk_cnt += $area_pdp_rwrk[$r];
         } else {		 
           while($rowrwrk     = mysql_fetch_row($mysql_rwrk)) {
                 ...
                 $area_pdp_rwrk[$r] = trim(stripslashes($rowrwrk[2]));
	       	     $tot_rwrk_hrs   = $tot_rwrk_hrs + $area_rwrk[$r];
                     $tot_pdp_rwrk_cnt += $area_pdp_rwrk[$r];
           }
		 }
	     for ($x=$qry0;$x>=1;$x=$x-1) {
              $query_ytd = " SELECT etl_id,ytd_count_rework, ytd_rework_hrs_pdp_cnt
                               FROM ".$name.".tgt_rework_hours_ytd 
                              WHERE etl_year = '$yr[$x]' 
                                AND area = '$pdparea[$r]' 
                          ";
