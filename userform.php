...
$queryx2      = "select issue_area_id,issue_area,short_desc,issue_area_ind,test_ind 
               ...
               order by issue_area ";
$mysql_datax2 = mysql_query($queryx2, $mysql_link) or die ("#77 Could not query: ".mysql_error());
...
$queryp = "select user_type_id,user_type 
           ...
           order by user_type  ";
$mysql_datap = mysql_query($queryp, $mysql_link) or die ("#66 Could not query: ".mysql_error());
...
if (isset($new)) {
	while (list($key) = each($new)) {
            ...
            //print($queryh);
            $mysql_datah = mysql_query($queryh, $mysql_link) or die ("#55 Could not query: ".mysql_error());
            ...
            if ($insert_ind == 0){
                ...
                //print($query);  
                $mysql_data = mysql_query($query, $mysql_link) or die ("#44 Could not query: ".mysql_error());
            } else {
...
if (isset($update)) {
    while (list($key) = each($update)) {
        ...
        $xissue_area_id[$key]    = addslashes($xissue_area_id[$key]); 	    
            ...
            //print($queryh);
            $mysql_datah = mysql_query($queryh, $mysql_link) or die ("#33 Could not query: ".mysql_error());
            $rowcnth = mysql_num_rows($mysql_datah);

        //if ($update_ind == 0){  
            $query2 = "select paswd from ".$name.".users where lanid = '$xlanid[$key]' "; 
            $mysql_data2 = mysql_query($query2, $mysql_link) or die ("#22 Could not query: ".mysql_error());
		    ...
		    $mysql_data = mysql_query($query, $mysql_link) or die ("#11 Could not query: ".mysql_error());
        //} else {
        //   echo "<script type=\"text/javascript\">window.alert(\"User Already Exists\")</script>";
        //}
	}
...
$query = "select a.user_id,UPPER(trim(a.lanid)),a.fullname,a.user_type_id,a.paswd,a.issue_area_id,user_ind  
            from ".$name.".users a, ".$name.".issue_areas b 
           where a.issue_area_id = b.issue_area_id 
             and UPPER(trim(a.lanid)) not in ('WEBMASTER','BACKOFFICE')            
...
while($row = mysql_fetch_row($mysql_data)) {
    ...
    $mysql_data1 = mysql_query($query1, $mysql_link) or die ("#2 Could not query: ".mysql_error());
	     ...
	     print("
               <tr rowspan=\"10\" valign=\"top\" scroll=\"yes\">
	            ...
	            <td align=\"center\" valign=\"middle\" bgcolor=\"#AFDCEC\">
                 <input type=\"hidden\" name=\"xlanid[$xid]\" value=\"$xlanid\">
                 <font color=\"#330099\">
                  $xlanid
                 </font>                 
	            </td>
