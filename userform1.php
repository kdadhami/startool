...
//print($querys5);
$mysql_data5 = mysql_query($querys5, $mysql_link) or die ("#session2 Could not query: ".mysql_error());
...
$queryx2      = "select issue_area_id,issue_area,short_desc,issue_area_ind,test_ind 
               ...
               order by issue_area ";
$mysql_datax2 = mysql_query($queryx2, $mysql_link) or die ("#101 Could not query: ".mysql_error());
...
$queryp = "select user_type_id,user_type 
           ...
           order by user_type  ";
$mysql_datap = mysql_query($queryp, $mysql_link) or die ("#102 Could not query: ".mysql_error());
...
if (isset($new)) {
	while (list($key) = each($new)) {
            ...
            //print($queryh);
            $mysql_datah = mysql_query($queryh, $mysql_link) or die ("#103 Could not query: ".mysql_error());
...
if (isset($update)) {
    while (list($key) = each($update)) {
        ...
        if ($update_ind == 0){  
            ...
            $mysql_data2 = mysql_query($query2, $mysql_link) or die ("#106 Could not query: ".mysql_error());
		    ...
		    $mysql_data = mysql_query($query, $mysql_link) or die ("#107 Could not query: ".mysql_error());
        } else {
                //if ( $update_ind == 1) {
                //     echo "<script type=\"text/javascript\">window.alert(\"User Already Exists\")</script>";
                //}
            ...
            <table border='0' align=\"center\" style=\"width=100%;\">
             <caption>$captn</caption>
              <tr>
                ...
                <td bgcolor=\"#99CCFF\" align=\"center\"><font color=\"#330099\">Lockdown</font</td>
...
$query = "select a.user_id,UPPER(trim(a.lanid)),a.fullname,a.user_type_id,a.paswd,a.issue_area_id,a.lockdown,user_ind 
            from ".$name.".users a, ".$name.".issue_areas b 
           where a.issue_area_id = b.issue_area_id
        order by b.issue_area,a.user_type_id,a.fullname "; 
$mysql_data = mysql_query($query, $mysql_link) or die ("#108 Could not query: ".mysql_error());
...
while($row = mysql_fetch_row($mysql_data)) {
    ...
    $mysql_data1 = mysql_query($query1, $mysql_link) or die ("#109 Could not query: ".mysql_error());
	...
	if ( $xuser_ind == 1 ) {
	     print("
          ...
          print("   </select>
                ...
                <td align=\"center\" valign=\"middle\" bgcolor=\"#AFDCEC\">
                 <select align=\"center\" name=\"xlockdown[$xid]\"> 
          ");
          $ldown[0] = "NO";
          $ldown[1] = "YES";
          $w = 0;
          for ($w=0;$w<=1; $w=$w+1) {
               if ($w == $xlockdown) {
                   print(" <option selected value=\"$w\">$ldown[$w]</option> ");
               }
               else {
                   print(" <option value=\"$w\">$ldown[$w]</option> ");
               }
          }
          print("   </select>
                   </td>
    ...
    for ($x=1;$x<=$num_records; ++$x) {
		 print("
          ...
          print("   </select>
                   ...
                   <td align=\"center\" valign=\"middle\" bgcolor=\"#AFDCEC\">
                    <select align=\"center\" name=\"new_lockdown[$x]\"> 
          ");
          $ldown[0] = "NO";
          $ldown[1] = "YES";
          $w = 0;
