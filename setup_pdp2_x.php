...
if (isset($update)) {
    while (list($key) = each($update)) {
           ...
           $xbuild_pw_ind[$key]   = addslashes($xbuild_pw_ind[$key]);
		   ///TEMP FIX FOR CORRINE - MISSING DATA - OCTOBER 3RD 2012
		   
 $query110_z = "INSERT into ".$name.".pdp_submit_logs(pdp_id,issue_area,updated_by,updated_on,module,period,followup)
                                     VALUES('$key','$uissue_area','$usr',current_timestamp,'SCOPING','$xpdp_period_id[$key]',1)";
          mysql_query($query110_z, $mysql_link) or die ("#110 Could not query: ".mysql_error());
		  ...
		  ///TEMP FIX FOR CORRINE - MISSING DATA - OCTOBER 3RD 2012
           ...
           if ($xpdp_category_id[$key] == -1){
  	       ...
  	       $query = "UPDATE ".$name.".pdp
				    ...
				    migration_ind   = '$xmigration_ind[$key]',
                                    build_pw_ind       = '$xbuild_pw_ind[$key]',
                                    etl_check       = '1'
		              WHERE pdp_id          = '$key' ";
		   //print($query);
           //print($key);
		   $mysql_data = mysql_query($query, $mysql_link) or die ("Could not query: ".mysql_error());
                   print("<b>Update successful for $xpdp_desc[$key].</b><br><br>");
    }
    //print($pdpid."key");
    if (isset($area_cnt)) {
        //print($area_cnt); 
...
if (isset($pdp) && ($start == 1) && ($submit != "Submit")) {
// -------------------------------------
   ...
   if ($rowcntx == 1) {
       ...
       while($rowx = mysql_fetch_row($mysql_datax)) {
			 ...
			 $xmigration_ind   = stripslashes($rowx[16]);
                         $xbuild_pw_ind   = stripslashes($rowx[17]);
             //$xpdp_period    = stripslashes($rowx[14]); 
			 $xv21buildstatus  = stripslashes($rowx[18]);
			 $xprogramcategory = stripslashes($rowx[19]);
			 $xsegment         = stripslashes($rowx[20]);
			 $xproductline     = stripslashes($rowx[21]);
			 $xlob             = stripslashes($rowx[22]);
            ...
            <table border='0' align=\"center\" scroll=\"yes\">
             ...
             <tr>
	          ...
	          <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:500px;\">
               <font color=\"#330099\"> 
                $xpdp_desc
               </font><input type=\"hidden\" name=\"xpdp_desc[$xid]\" value=\"$xpdp_desc\">
              </td>
              <td align=\"center\" valign=\"middle\" bgcolor=\"#E8E8E8\" style=\"width:100px;\">
              </td>	          
             </tr>
     ...
     // ************************** PDP Category (END) - Build (START) **********************************
       print("       
              ...
              <td bgcolor=\"#CCFFFF\" align=\"center\" style=\"width:100px;\"><font color=\"#330099\">$zcategory_scope</font></td>
                     ...
                     <tr>
          <td bgcolor=\"#99CCFF\" align=\"center\"><font color=\"#330099\">Build Paperwork</font></td>
          <td align=\"center\" valign=\"middle\" bgcolor=\"#CCFFFF\" style=\"width:500px;\">
       ");
       if ($xbuild_pw_ind[$y] == 0) {
           print(" <select align=\"center\" name=\"xbuild_pw_ind[$xid]\" style=\"color: #000000; font-weight: normal; background-color: #FFFF00;\"> ");
       } else {
           print(" <select align=\"center\" name=\"xbuild_pw_ind[$xid]\" style=\"color: #000000; font-weight: normal; background-color: #FFFFFF;\"> ");
       }
       for ($e=0;$e<=2; ++$e) {
            if ($xbuild_pw_ind == $ind_id2[$e]) {
                print(" <option selected value=\"$ind_id2[$e]\">$ind2[$e]</option> "); 
            } else {
                print(" <option value=\"$ind_id2[$e]\">$ind2[$e]</option> ");    
            }   
       }  
     // ************************** Build (END) - Complexity (START)****************************
