...
$queryx2      = "select issue_area_id,issue_area,short_desc,issue_area_ind,test_ind 
                  ...
                  where issue_area_ind = 1 
                     or UPPER(trim(issue_area)) = 'SIGNOUT'
               order by issue_area ";
...
$queryp = " select a.user_type_id,a.user_type,a.issue_area_id,b.issue_area 
              from ".$name.".user_types a, ".$name.".issue_areas b  
             where a.issue_area_id = b.issue_area_id
               and b.issue_area = '$uissue_area'
               and a.lockdown   = 0 
          order by a.user_type ";
//                or 
//                  (
//                   UPPER(trim(a.user_type)) = 'GUEST' 
//                   and
//                   a.issue_area_id = b.issue_area_id
//                  )                    
//print($queryp);          
$mysql_datap = mysql_query($queryp, $mysql_link) or die ("#4 Could not query: ".mysql_error());
...
if (isset($new)) {
	while (list($key) = each($new)) {
	        ...
	        $new_pswd[$key]          = addslashes($new_pswd[$key]);
	        $new_issue_area_id[$key] = addslashes($new_issue_area_id[$key]); 
	        $new_lockdown[$key]      = addslashes( $new_lockdown[$key]);

            $queryh = "SELECT lanid,paswd 
                         FROM users 
                        WHERE UPPER(trim(lanid)) = '$new_lanid[$key]'";
            //print($queryh);
            $mysql_datah = mysql_query($queryh, $mysql_link) or die ("#5 Could not query: ".mysql_error());
            ...
            if ($insert_ind == 0){
                $query = "INSERT into ".$name.".users(lanid,fullname,user_type_id,paswd,issue_area_id,lockdown)
                          VALUES('$new_lanid[$key]','$new_fullname[$key]','$new_user_type_id[$key]',
                                  sha1('$new_paswd[$key]'),'$new_issue_area_id[$key]','$new_lockdown[$key]')";
                //print($query);  
...
if (isset($update)) {
    while (list($key) = each($update)) {
        ...
        $xlockdown[$key]         = addslashes( $xlockdown[$key]); 	    

        $queryh = "SELECT user_id,UPPER(trim(lanid)),paswd 
                     FROM users 
                    WHERE UPPER(trim(lanid)) = '$xlanid[$key]'
                      AND user_id <> $key ";
        //print($queryh);
        $mysql_datah = mysql_query($queryh, $mysql_link) or die ("#7 Could not query: ".mysql_error());
        $rowcnth = mysql_num_rows($mysql_datah);

        while($rowh = mysql_fetch_row($mysql_datah)) {
              $zid     = stripslashes($rowh[0]);
              $zlanid  = stripslashes(strtoupper(trim($rowh[1])));
              $zpaswd  = stripslashes($rowh[2]);
        }

        // row with same unique key already exist 
        if ($rowcnth == 1) {
            //print("I am 1");
            if ($zid <> $key ) { 
                $update_ind  = 1;
        ...
        // Multiple unique key exists, have to delete or update one             
        //if ($rowcnth > 1){
        //    print("I am many");
        //    $update_ind = 0;
        //} 
        ...
        if ($update_ind == 0){  
		    ...
		    $mysql_data = mysql_query($query, $mysql_link) or die ("#8 Could not query: ".mysql_error());
                ...
                if ( $update_ind == 3) {
                     echo "<script type=\"text/javascript\">window.alert(\"Multiple rows with same userids exists\")</script>";
                }
        }
	}
}
...
print("<html>
        ...
        <body>
         <div id=\"content\">
          <form method=\"post\" action=\"./rolesform.php\">
...
$query = "select a.user_id,UPPER(trim(a.lanid)),a.fullname,a.user_type_id,a.paswd,a.issue_area_id,lockdown,a.user_ind 
            from ".$name.".users a, ".$name.".issue_areas b 
           where a.issue_area_id = b.issue_area_id
             and a.issue_area_id = '$uissue_area_id'
             and a.lockdown = 0 
             and UPPER(trim(a.lanid)) not in ('UAT','CM','MO','RA','UAT USER','CM USER','MO USER','RA USER','WEBMASTER','BACKOFFICE')  
        order by b.issue_area,a.user_type_id,a.fullname "; 
$mysql_data = mysql_query($query, $mysql_link) or die ("#9 Could not query: ".mysql_error());
...
while($row = mysql_fetch_row($mysql_data)) {
    ...
    $xlockdown      = stripslashes($row[6]);
    $xuser_ind      = stripslashes($row[7]);
	

    $query1      = "select UPPER(trim(created_by)) from ".$name.".issues where UPPER(trim(created_by)) = '$xlanid' ";
    $mysql_data1 = mysql_query($query1, $mysql_link) or die ("#10 Could not query: ".mysql_error());
	     ...
	     print("   
		       <tr rowspan=\"10\" valign=\"top\" scroll=\"yes\">
	            ...
	            <td align=\"center\" valign=\"middle\" bgcolor=\"#AFDCEC\">
                 ...
                 <input type=\"hidden\" name=\"xlanid2[$xid]\" value=\"$xlanid2\">
                 <input type=\"hidden\" name=\"xlockdown[$xid]\" value=\"$xlockdown\">
	            </td>
	            <td align=\"center\" valign=\"middle\" bgcolor=\"#AFDCEC\">
                 <input type=\"password\" name=\"xpaswd[$xid]\" value=\"$xpaswd\" size=\"20\" maxlength=\"50\">
                 <input type=\"hidden\" name=\"xpaswd2[$xid]\" value=\"$xpaswd2\">
	            </td>
	            <td align=\"center\" valign=\"middle\" bgcolor=\"#AFDCEC\">
...
if ($submit == "Add") {
    for ($x=1;$x<=$num_records; ++$x) {
		 print("
                <tr>
		          <td align=\"center\" valign=\"middle\" bgcolor=\"#99CC00\">
		           <input type=\"hidden\" name=\"new_lockdown[$x]\" value=\"0\">
		          </td>
		          <td align=\"center\" valign=\"middle\" bgcolor=\"#E8E8E8\">
		          </td>		          
